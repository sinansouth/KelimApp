...
import { AppSettings, getAppSettings, saveAppSettings, getUserProfile, saveUserProfile, clearLocalUserData, markTutorialAsSeen } from '../services/userService';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose, onOpenFeedback, onOpenAdmin, onRestartTutorial }) => {
    ...
    const handleAdminLogin = async (e: React.FormEvent) => {
        ...
        if (adminPassword === "Adminullah5510.") {
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            ...
            >
                ...
                <div className="p-5 space-y-4">
                    ...
                    <button
                        onClick={() => {
                            if (window.confirm("TÜM İLERLEMENİZ SİLİNECEK! Bu işlem geri alınamaz. Emin misiniz?")) {
                                clearLocalUserData();
                                window.location.reload();
                            }
                        }}
                        className="w-full p-3 rounded-2xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-900/10 flex items-center gap-3 text-left"
                    >
                        <div className="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 flex items-center justify-center">
                            <X size={20} />
                        </div>
                        <div>
                            <div className="font-bold text-sm text-red-600">İlerlemeyi Sıfırla</div>
                            <div className="text-[10px] text-red-500/70">Her şeyi sil ve baştan başla</div>
                        </div>
                    </button>
