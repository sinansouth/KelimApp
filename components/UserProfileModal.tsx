...
import { X, Trophy, Flame, Clock, ShieldCheck, Gamepad2, ChevronDown, ChevronUp, Target } from 'lucide-react';
import { getPublicUserProfile } from '../services/firebase';
import { AVATARS, FRAMES, BACKGROUNDS, BADGES } from '../data/assets';
...
const UserProfileModal: React.FC<UserProfileModalProps> = ({ userId, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 pb-6 -mt-16 flex flex-col items-center overflow-y-auto custom-scrollbar flex-1 w-full">
            ...
            <div className="relative w-28 h-28 flex items-center justify-center mb-3 shrink-0">
                ...
                <div className={`absolute inset-0 w-full h-full rounded-full z-10 ${bgDef.style} border-4 border-white dark:border-slate-900`}></div>
            ...
            <h2 className="text-2xl font-black text-slate-800 dark:text-white text-center">{userData.name}</h2>
            ...
            <div className="grid grid-cols-3 gap-3 w-full mb-6">
                <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center border border-slate-100 dark:border-slate-700">
                     <div className="flex justify-center text-orange-500 mb-1"><Flame size={20} className="fill-current" /></div>
                     <div className="text-lg font-black text-slate-800 dark:text-white">{userData.streak || 0}</div>
                     <div className="text-[10px] font-bold text-slate-400 uppercase">Gün Seri</div>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center border border-slate-100 dark:border-slate-700">
                     <div className="flex justify-center text-yellow-500 mb-1"><Trophy size={20} className="fill-current" /></div>
                     <div className="text-lg font-black text-slate-800 dark:text-white">{userData.xp || 0}</div>
                     <div className="text-[10px] font-bold text-slate-400 uppercase">Toplam XP</div>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-2xl text-center border border-slate-100 dark:border-slate-700">
                     <div className="flex justify-center text-blue-500 mb-1"><Clock size={20} /></div>
                     <div className="text-lg font-black text-slate-800 dark:text-white">{Math.floor((userData.totalTimeSpent || 0) / 60)}</div>
                     <div className="text-[10px] font-bold text-slate-400 uppercase">Saat</div>
                </div>
            </div>
            ...
            <div className="w-full mb-6">
                ...
                {unlockedBadges.length > 0 ? (
                    <div className="flex flex-col items-center">
                        <div className="grid grid-cols-5 gap-2 w-full">
                             {displayedBadges.map((badge: Badge) => (
                                ...
                                >
                                    <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-xl shadow-sm cursor-pointer hover:scale-110 transition-transform relative">
                        ...
                        {unlockedBadges.length > 5 && (
                            <button 
                                ...
                                className="mt-3 text-xs font-bold text-indigo-500 flex items-center gap-1 hover:underline"
                            >
                    ...
                    <div className="text-center text-xs text-slate-400 py-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
            ...
            <div className="w-full">
                ...
                <div className="space-y-2">
                     ...
                     <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                         <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Eşleştirme Rekoru</span>
                         <span className="text-sm font-black text-indigo-600 dark:text-indigo-400">{userData.matchingBestTime || 0} P</span>
                     </div>
                     <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                         <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Yazma Rekoru</span>
                         <span className="text-sm font-black text-orange-600 dark:text-orange-400">{userData.typingHighScore || 0} P</span>
                     </div>
                     <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                         <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Kelime Türetmece</span>
                         <span className="text-sm font-black text-purple-600 dark:text-purple-400">{userData.chainHighScore || 0} P</span>
                     </div>
                     <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                         <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Labirent</span>
                         <span className="text-sm font-black text-red-600 dark:text-red-400">{userData.mazeHighScore || 0} P</span>
                     </div>
                     <div className="flex justify-between items-center p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                         <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Bulmaca</span>
                         <span className="text-sm font-black text-blue-600 dark:text-blue-400">{userData.wordSearchHighScore || 0} P</span>
                     </div>
                </div>
            </div>
